...
namespace w2l
{
  namespace streaming
  {
    ...
    DecoderFactory::DecoderFactory(
        ...
        : letterMap_(fl::lib::text::Dictionary(letterDictFile)),
          ...
          transitions_(transitions)
    {
      ...
      LOG(INFO) << "[Letters] " << alphabetSize_ << " tokens loaded.\n";
      silence_ = letterMap_.getIndex(silenceToken);
      blank_ =
          letterMap_.contains("#") ? letterMap_.getIndex("#") : -1;
      ...
      if (!wordDictFile.empty())
      {
        ...
        LOG(INFO) << "[Words] " << nWords << " words loaded.\n";
        unk_ = wordMap_.getIndex("<unk>");
      }
    ...
    Decoder DecoderFactory::createDecoder(const fl::lib::text::LexiconDecoderOptions &opt) const
    {
    ...
    Decoder DecoderFactory::createDecoder(const fl::lib::text::LexiconFreeDecoderOptions &opt) const
    {
